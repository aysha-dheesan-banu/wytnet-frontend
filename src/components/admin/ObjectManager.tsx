...
const ObjectManager: React.FC<ObjectManagerProps> = ({ createTrigger, onTriggerHandled }) => {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      toast.error(message || 'Error saving object');
    }
  });
  ...
  const renderParents = (obj: WytObject) => {
    ...
    return (
      <div className="text-sm font-bold text-gray-900 dark:text-white ml-0.5">
        {count}
      </div>
    );
  };
    ...
    <div className="p-8">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 bg-gray-50/50 dark:bg-slate-900/30 p-2 rounded-2xl border border-gray-100 dark:border-slate-800">
        ...
        <select
          ...
          className="px-4 py-3 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl text-xs font-semibold uppercase tracking-widest text-gray-500 outline-none focus:ring-2 focus:ring-indigo-500/10 transition-all"
        >
      ...
      <div className="overflow-x-auto rounded-[2rem] border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-800 shadow-sm">
        <table className="w-full text-left font-sans">
          <thead className="bg-gray-50 dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700">
            <tr>
              <th className="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Object Icon</th>
              ...
              <th className="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Childs</th>
            ...
            {loading ? (
              ...
              paginatedObjects.map(obj => (
                <tr key={obj.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-700/20 transition-all group">
                  ...
                  <td className="px-6 py-4">
                    <span className="px-3 py-1 bg-gray-100 dark:bg-slate-900 text-gray-600 dark:text-gray-400 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-gray-200 dark:border-slate-700">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm font-bold text-gray-900 dark:text-white">{obj.child_count || 0}</span>
                  </td>
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[3rem] shadow-2xl p-12 border border-gray-100 dark:border-slate-700 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2.5 ml-1">Unique Name</label>
                  <input
                    type="text"
                    value={formData.name}
                    ...
                    className="w-full px-6 py-4 bg-gray-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white rounded-2xl text-sm font-semibold outline-none transition-all dark:text-white"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2.5 ml-1">Classification Type</label>
                  <select
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2.5 ml-1">Visual Icon</label>
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      <input
                        ...
                        className="w-full px-4 py-2 bg-gray-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white rounded-xl text-xs font-semibold outline-none transition-all dark:text-white"
                      />
                      <label className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-bold uppercase transition-colors ${isUploading ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 cursor-pointer hover:bg-indigo-100'}`}>
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2.5 ml-1">System Description</label>
                  <textarea
                    value={formData.description}
                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    rows={8}
                    placeholder="Provide a detailed description of this object..."
                    className="w-full px-6 py-4 bg-gray-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white rounded-2xl text-sm font-semibold outline-none transition-all dark:text-white resize-none"
                  />
                </div>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Parent Nodes</label>
                  ...
                  <div className="space-y-3">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Child Nodes</label>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    <div className="flex-1 p-4 bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl">
                      <span className="block text-[8px] font-bold text-gray-400 uppercase mb-1">Total Children</span>
                      <span className="text-xl font-bold text-gray-900 dark:text-white">{viewingObject.child_count || 0}</span>
                    </div>
                    <div className="flex-1 p-4 bg-gray-50/50 dark:bg-slate-800/30 rounded-2xl">
                      <span className="block text-[8px] font-bold text-gray-400 uppercase mb-1">Parent Count</span>
