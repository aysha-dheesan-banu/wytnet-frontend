...
import { ObjectItem, Post } from '../api/types';
...
const EditPostModal: React.FC<EditPostModalProps> = ({ post, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePost(post.id, {
        ...
        allow_response: formData.allow_response
      });
      onSuccess();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
      <div className="bg-white rounded-[2.5rem] w-full max-w-5xl max-h-[92vh] overflow-y-auto shadow-2xl animate-in zoom-in fade-in duration-300 border border-white/20">
        <div className="p-8 pb-4 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-sm z-20">
          <div>
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Edit Post</h2>
            <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mt-1">Update your intent details below.</p>
          </div>
        ...
        <div className="px-8 pb-10">
          {isRestricted && (
            <div className="bg-[#FFF9EB] border border-[#FFE7A3] rounded-3xl p-6 mb-8 flex items-start gap-4 animate-in slide-in-from-top-4 duration-500">
               ...
               <div>
                 <h4 className="text-amber-900 font-black text-[13px] uppercase tracking-widest mb-1.5">Restricted Edit Mode</h4>
            ...
            <div className="space-y-8">
              <div>
                <div 
                  ...
                  className={`border-4 border-dashed rounded-[2.5rem] h-80 flex flex-col items-center justify-center transition-all overflow-hidden relative group ${isRestricted ? 'border-gray-50 bg-gray-50/50 cursor-default' : 'border-gray-100 bg-gray-50/30 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/50'}`}
                >
                  <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" disabled={isRestricted} />
                  {previewUrl ? (
                    <>
                      <img src={previewUrl.startsWith('data:') || previewUrl.startsWith('blob:') ? previewUrl : `http://localhost:8000/${previewUrl}`} alt="Preview" className="w-full h-full object-cover" />
                      {!isRestricted && (
                    ...
                    <div className="text-center">
                       ...
                       <p className="text-gray-400 font-black text-[10px] uppercase tracking-[0.2em]">No image attached</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-50/50 rounded-[2rem] p-8 border border-gray-100/50">
                <h3 className="text-indigo-900 font-black text-[11px] uppercase tracking-widest mb-6">Post Interactions</h3>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { key: 'allow_response', label: 'Allow Response', icon: '⚡' }
                  ].map(interaction => (
                    <label key={interaction.key} className="flex items-center justify-between group cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-gray-700">{interaction.label}</span>
                      </div>
            ...
            <div className="space-y-8">
              <div className="flex bg-gray-50 p-1.5 rounded-[1.5rem] border border-gray-100">
                <button 
                  ...
                  className={`flex-1 py-4 rounded-[1.2rem] text-[11px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${formData.post_type === 'NEED' ? 'bg-white text-emerald-600 shadow-xl shadow-emerald-100/50 border border-emerald-50' : 'text-gray-400 opacity-60'}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 py-4 rounded-[1.2rem] text-[11px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${formData.post_type === 'OFFER' ? 'bg-white text-blue-600 shadow-xl shadow-blue-100/50 border border-blue-50' : 'text-gray-400 opacity-60'}`}
                >
              ...
              <div className="relative" ref={searchRef}>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 px-1">What are you looking for? *</label>
                <div className="relative">
                  {isRestricted ? (
                    <div className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl flex items-center gap-4 text-gray-400 font-bold">
                    ...
                    <>
                      <input 
                        ...
                        className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-100 outline-none transition-all font-bold text-gray-900 placeholder:text-gray-300"
                        ...
                        <div className="absolute top-full left-0 w-full mt-3 bg-white rounded-3xl shadow-2xl border border-gray-50 z-50 overflow-hidden animate-in slide-in-from-top-4 duration-200">
                          {objectResults.map(obj => (
                            <div 
                              ...
                              className="px-6 py-4 hover:bg-indigo-50 cursor-pointer text-sm font-bold text-gray-700 transition-colors border-b border-gray-50 last:border-none"
                            >
                              {obj.name}
                            </div>
                          ))}
              ...
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 px-1">Where is this? *</label>
                {isRestricted ? (
                  <div className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl flex items-center gap-4 text-gray-400 font-bold">
                  ...
                  <input 
                    ...
                    className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-100 outline-none transition-all font-bold text-gray-900"
              ...
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 px-1">Post title *</label>
                <input 
                  type="text" 
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-100 outline-none transition-all font-bold text-gray-900 min-h-[120px] resize-none"
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  type="button"
                  onClick={onClose}
                  className="px-8 py-4 bg-gray-50 text-gray-400 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-gray-100 transition-all active:scale-95"
                >Cancel</button>
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-2xl shadow-indigo-200 hover:bg-indigo-700 disabled:opacity-50 transition-all transform active:scale-[0.98]"
                >
