   ...
   return (
      <div className="bg-wyt-card-gradient rounded-[32px] p-8 text-white shadow-xl shadow-indigo-100/50 sticky top-24">
         <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold mb-1 tracking-tight">{user?.full_name || user?.name || 'User'}</h3>
            <p className="text-white/60 text-sm mb-8 font-medium">@{(user?.full_name || user?.username || 'user').toLowerCase().replace(/\s+/g, '')}</p>
