    ...
    <div className="min-h-screen bg-[#f8fafc] text-gray-800 font-['Inter'] flex flex-col">
      ...
      <header className="bg-white border-b border-gray-100 py-3 px-6 flex justify-between items-center sticky top-0 z-50 shadow-[0_1px_2px_0_rgba(0,0,0,0.03)]">
        ...
        <div className="flex items-center gap-5">
          ...
          <button onClick={goToLogin} className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-all shadow-sm">
            Login / Join
          </button>
        </div>
      </header>
      ...
      <main className="max-w-[1400px] mx-auto w-full px-6 py-6 grid grid-cols-1 md:grid-cols-12 gap-6 flex-1">
        ...
        <aside className="md:col-span-2 space-y-4">
          <h2 className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest pl-1">Filter By Location</h2>
          <div className="relative">
            ...
            <input
              className="block w-full pl-9 pr-3 py-2.5 border border-gray-100 rounded-xl text-sm bg-gray-50/50 focus:ring-wyt-purple focus:border-wyt-purple placeholder-gray-400 outline-none"
        ...
        <section className="md:col-span-8 space-y-4">
          ...
          <div className="bg-wyt-purple rounded-2xl p-1.5 flex items-center gap-3 text-white shadow-lg overflow-hidden">
            <div className="flex items-center gap-2 font-medium px-4 py-2.5 bg-white/10 rounded-xl hover:bg-white/20 transition-all cursor-pointer">
            ...
            <div className="relative flex-1">
              ...
              <input
                className="block w-full pl-10 pr-4 py-2.5 bg-white/5 border-none rounded-xl text-sm text-white placeholder-white/50 focus:ring-2 focus:ring-white/10 transition-all outline-none"
                placeholder="Search..."
                type="text"
                value={searchQuery}
            ...
            <button onClick={goToLogin} className="bg-white text-wyt-purple font-semibold px-6 py-2.5 rounded-xl text-sm flex items-center gap-2 hover:bg-gray-50 transition-all active:scale-95 shadow-sm">
          ...
          <div className="bg-white border border-gray-50 rounded-[2rem] p-5 shadow-sm flex flex-col md:flex-row items-center gap-5 transition-all hover:shadow-md">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-semibold text-gray-900 tracking-tight">Welcome to WytWall</h1>
              ...
              <div className="flex flex-wrap gap-3 mt-3">
                ...
                <div className="flex items-center gap-1.5 text-wyt-purple text-[10px] font-medium">
          ...
          {loading && !posts.length ? (
            ...
            <div className="flex flex-col gap-4">
              {posts.filter(post => {
                ...
                const matchesLocation = locationQuery ? (post.location || '').toLowerCase().includes(locationQuery.toLowerCase()) : true;
                // Also hide expired/completed for visitors
                const isExpired = post.valid_until && new Date(post.valid_until) < new Date();
                return matchesSearch && matchesLocation && !isExpired;
              }).map(post => {
                ...
                return (
                  <article key={post.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.04)] hover:shadow-md transition-all">
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-indigo-900 rounded-xl flex items-center justify-center text-white font-medium text-xs uppercase">
                          {(user?.full_name || user?.username)?.[0] || 'U'}
                        </div>
                        <div>
                          <h3 className="font-semibold text-gray-900 text-xs">{user?.full_name || user?.username || 'User'}</h3>
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`${typeBg} px-2.5 py-1 rounded-lg text-[8px] font-semibold uppercase tracking-widest`}>{post.post_type}</span>
                        <span className="bg-gray-50 text-gray-400 px-2.5 py-1 rounded-lg text-[8px] font-semibold uppercase tracking-widest">Active</span>
                        <button onClick={goToLogin} className="ml-1 text-blue-600 text-[9px] font-medium flex items-center gap-1 border border-blue-100 px-3 py-1 rounded-full hover:bg-blue-600 hover:text-white transition-all whitespace-nowrap">
                          + Follow
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="pl-0.5 mb-1">
                      <div className="flex gap-4">
                        <div className="flex-1">
                          <h4 className="text-base font-semibold text-gray-900 mb-0.5 leading-tight">{post.title}</h4>
                    ...
                    <button
                      onClick={goToLogin}
                      className="w-full hover:opacity-90 text-white font-medium py-1.5 rounded-lg flex items-center justify-center gap-2 transition-all shadow-md active:scale-[0.98] text-xs bg-[#10b981]"
                    >
        ...
        <aside className="md:col-span-2 space-y-6">
          <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm sticky top-24 text-center">
            ...
            <h2 className="text-sm font-semibold text-gray-900 leading-tight">Join and Get WytPass</h2>
            ...
            <button onClick={goToLogin} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-1.5 rounded-lg flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 text-[10px]">
              Sign Up Free
      ...
      <footer className="bg-white border-t border-gray-100 mt-16 py-12">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          {[
            ...
            <div key={group.title}>
              <h4 className="text-[10px] font-semibold text-gray-900 uppercase tracking-[0.2em] mb-6">{group.title}</h4>
        ...
        <div className="max-w-7xl mx-auto border-t border-gray-50 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 px-6">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400 font-medium">
            ...
            <span className="text-indigo-600 font-semibold">WytPass</span>
